...
export async function GET(req: Request): Promise<NextResponse> {
  ...
  const events = clustered.map(({ canonical, corroborated_by, source_count, confidence }) => {
    ...
    return {
      ...
      source: typeof base.source === 'string' ? base.source : sanitizeEventForClient(base).outlet_name,
      source_id: typeof base.source_id === 'string' ? base.source_id : sanitizeEventForClient(base).source_url,
      severity: typeof base.severity === 'number' ? Math.max(1, Math.min(4, base.severity)) : null,
      ...
      summary_full: typeof base.summary_full === 'string' ? base.summary_full : null,
