...
export default function TravelCheckinPage() {
  ...
  const submit = async () => { try { const res = await fetch(`/api/v1/travel?token=${encodeURIComponent(token)}`, { method: 'PATCH' }); if (!res.ok) throw new Error(`HTTP ${res.status}`); const json = await res.json() as { success: boolean; error?: string }; setStatus(json.success ? 'Check-in recorded.' : (json.error ?? 'Check-in failed.')) } catch { setStatus('Network error — please try again.') } }
