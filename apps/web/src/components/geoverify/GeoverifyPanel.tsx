...
export function GeoverifyPanel() {
  ...
  useEffect(() => {
    const loadSubmissions = async () => {
      try {
        setError(null)
        const r = await fetch('/api/v1/geoverify', { cache: 'no-store' })
        if (!r.ok) throw new Error(`Failed to load submissions: ${r.statusText}`)
        const json = await r.json() as { data?: Submission[] }
        setSubmissions(json.data ?? [])
        setSelectedSub((json.data ?? [])[0] ?? null)
      } catch (err) {
  ...
  const submit = async () => {
    try {
      ...
      if (!res.ok) throw new Error(`Failed to submit evidence: ${res.statusText}`)
      ...
      setSubmitError(err instanceof Error ? err.message : 'Failed to submit evidence')
    }
  }
  const exportReport = () => {
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[1fr_1.1fr]">
      {error && (
        <div className="col-span-full rounded-lg border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-400">
